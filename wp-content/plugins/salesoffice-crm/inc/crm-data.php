	...
	function pera_crm_get_recent_leads( int $limit = 20 ): array {
		...
		foreach ( $ids as $lead_id ) {
			...
			$rows[]     = array(
				...
				'url'        => function_exists( 'pera_crm_get_client_view_url' ) ? pera_crm_get_client_view_url( $lead_id ) : home_url( '/so/crm/client/' . $lead_id . '/' ),
			);
	...
	function pera_crm_get_tasks_view_data(): array {
		...
		foreach ( $raw_rows as $row ) {
			...
			$view_row = array(
				...
				'client_url'    => function_exists( 'pera_crm_get_client_view_url' ) ? pera_crm_get_client_view_url( $client_id ) : home_url( '/so/crm/client/' . $client_id . '/' ),
				'due_at'        => $due_at,
				'due_ts'        => $due_ts,
	...
	function pera_crm_get_leads_view_data( int $page = 1, int $per_page = 20, string $derived_type = 'lead', string $list_view = 'leads' ): array {
		...
		foreach ( $offset_ids as $lead_id ) {
			...
			$items[] = array(
				...
				'crm_url'          => function_exists( 'pera_crm_get_client_view_url' ) ? pera_crm_get_client_view_url( $lead_id ) : home_url( '/so/crm/client/' . $lead_id . '/' ),
