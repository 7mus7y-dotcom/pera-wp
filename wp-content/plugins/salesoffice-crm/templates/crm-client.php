...
$client_id = function_exists( 'pera_crm_client_view_get_client_id' ) ? pera_crm_client_view_get_client_id() : (int) get_query_var( 'salesoffice_client_id', 0 );
...
$frontend_url = function_exists( 'pera_crm_client_view_url' ) ? pera_crm_client_view_url( $client_id ) : home_url( '/so/crm/client/' . $client_id . '/' );
$clients_url  = home_url( '/so/crm/clients/' );
$leads_url    = home_url( '/so/crm/leads/' );
    ...
    <div class="content-panel-box border-dm">
	  ...
	  <a href="#crm-add-reminder" class="crm-floating-add-task" aria-label="<?php esc_attr_e( 'Add task', 'hello-elementor-child' ); ?>">
		<svg class="icon" aria-hidden="true">
		  <use href="<?php echo esc_url( SALESOFFICE_CRM_URL . 'assets/logos-icons/icons.svg#icon-check' ); ?>"></use>
		</svg>
	  </a>
	  <?php endif; ?>
</main>
