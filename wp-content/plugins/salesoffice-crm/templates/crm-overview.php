...
$crm_current_url    = home_url( wp_unslash( (string) ( $_SERVER['REQUEST_URI'] ?? '/so/crm/' ) ) );
$new_lead_url       = home_url( '/so/crm/new/' );
$overview_task_cap  = 8;
          ...
          <?php else : ?>
            ...
            <?php foreach ( $today_cards['tasks'] as $task ) : ?>
              <article class="card-shell crm-task-card">
                <a class="btn btn--ghost btn--blue crm-task-client-btn" href="<?php echo esc_url( home_url( '/so/crm/client/' . (int) $task['lead_id'] . '/' ) ); ?>"><?php echo esc_html( (string) ( $task['lead_name'] ?: __( 'Untitled lead', 'hello-elementor-child' ) ) ); ?></a>
            ...
            <?php if ( $today_cards['has_more'] ) : ?>
              <article class="card-shell crm-task-card crm-task-card--more">
                ...
                <a class="btn btn--ghost btn--blue" href="<?php echo esc_url( home_url( '/so/crm/tasks/' ) ); ?>"><?php echo esc_html__( 'See all tasks', 'hello-elementor-child' ); ?></a>
              </article>
            <?php endif; ?>
            </div>
          <?php endif; ?>
        ...
        <article class="card-shell">
          ...
          <?php else : ?>
            ...
            <?php foreach ( $overdue_cards['tasks'] as $task ) : ?>
              <article class="card-shell crm-task-card">
                <a class="btn btn--ghost btn--red crm-task-client-btn" href="<?php echo esc_url( home_url( '/so/crm/client/' . (int) $task['lead_id'] . '/' ) ); ?>"><?php echo esc_html( (string) ( $task['lead_name'] ?: __( 'Untitled lead', 'hello-elementor-child' ) ) ); ?></a>
      ...
      <section class="section" aria-labelledby="crm-kpi-heading">
        ...
        <div class="grid-3 crm-kpi-grid cards-slider cards-slider--snap cards-slider--grid-lg" aria-label="<?php echo esc_attr__( 'CRM Pipeline Overview', 'hello-elementor-child' ); ?>">
	        ...
	        <article class="card-shell" data-crm-push-card>
	          ...
	          <div class="crm-task-action" style="display:flex;gap:8px;flex-wrap:wrap;">
	            ...
	            <form method="post" action="<?php echo esc_url( home_url( '/wp-admin/admin-post.php' ) ); ?>">
	              ...
	              <input type="hidden" name="peracrm_redirect" value="<?php echo esc_url( home_url( '/so/crm/' ) ); ?>">
	      ...
	      <?php endif; ?>
			...
			$tasks_page_url = home_url( wp_unslash( (string) ( $_SERVER['REQUEST_URI'] ?? '/so/crm/tasks/' ) ) );
			?>
      ...
      <div class="crm-leads-toolbar">
        ...
        <div class="crm-toolbar-actions">
          <div class="crm-type-toggle" role="group" aria-label="<?php echo esc_attr__( 'Lead or client listing', 'hello-elementor-child' ); ?>">
            <a class="btn <?php echo esc_attr( 'leads' === $clients_type_view ? 'btn--solid' : 'btn--ghost' ); ?> btn--blue" href="<?php echo esc_url( add_query_arg( 'type', 'leads', home_url( '/so/crm/clients/' ) ) ); ?>"><?php esc_html_e( 'Leads', 'hello-elementor-child' ); ?></a>
            <a class="btn <?php echo esc_attr( 'clients' === $clients_type_view ? 'btn--solid' : 'btn--ghost' ); ?> btn--blue" href="<?php echo esc_url( add_query_arg( 'type', 'clients', home_url( '/so/crm/clients/' ) ) ); ?>"><?php esc_html_e( 'Clients', 'hello-elementor-child' ); ?></a>
            <a class="btn <?php echo esc_attr( $is_inactive ? 'btn--solid' : 'btn--ghost' ); ?> btn--blue" href="<?php echo esc_url( add_query_arg( 'type', 'inactive', home_url( '/so/crm/clients/' ) ) ); ?>"><?php esc_html_e( 'Inactive', 'hello-elementor-child' ); ?></a>
          </div>
        ...
        <article class="card-shell">
          ...
          <p><a class="btn btn--ghost btn--blue" href="<?php echo esc_url( (string) $lead['crm_url'] ); ?>"><?php echo esc_html__( 'View Lead', 'hello-elementor-child' ); ?></a></p>
			...
			$pagination = paginate_links(
				array(
					'base'      => trailingslashit( home_url( '/so/crm/clients/%_%' ) ),
					'format'    => 'page/%#%/',
			...
			if ( is_string( $pagination ) ) {
				$pagination = str_replace( '/so/crm/clients/page/1/', '/so/crm/clients/', $pagination );
