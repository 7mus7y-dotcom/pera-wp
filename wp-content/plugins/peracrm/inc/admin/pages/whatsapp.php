...
function peracrm_render_whatsapp_page()
{
    ...
    if (!empty($settings['test_mode'])) {
        echo '<div class="notice notice-warning inline"><p><strong>' . esc_html__('META TEST MODE', 'peracrm') . '</strong> — ' . esc_html__('This configuration is intended for Meta test WABA and test Phone Number ID assets.', 'peracrm') . '</p></div>';
    }
    ...
    if (isset($_GET['enquiry_test'])) {
        ...
        echo '<div class="notice peracrm-whatsapp-admin__notice ' . ($ok ? 'notice-success' : 'notice-warning') . '"><p>' . esc_html($ok ? 'Enquiry notification test sent to both channels.' : 'Enquiry notification test executed with at least one failed channel.') . '</p></div>';
    }

    echo '<section class="peracrm-whatsapp-admin__section peracrm-whatsapp-admin__section--diagnostics">';
    echo '<h2>' . esc_html__('Webhook diagnostics', 'peracrm') . '</h2>';
    echo '<table class="widefat striped peracrm-whatsapp-table peracrm-whatsapp-table--diagnostics"><tbody>';
    ...
    echo '<section class="peracrm-whatsapp-admin__section peracrm-whatsapp-admin__section--settings">';
    echo '<h2>' . esc_html__('Integration settings', 'peracrm') . '</h2>';
    echo '<form method="post" action="' . esc_url(admin_url('admin-post.php')) . '" class="peracrm-whatsapp-admin__form peracrm-whatsapp-admin__form--settings">';
    ...
    echo '<tr><th scope="row">' . esc_html__('Business WhatsApp number', 'peracrm') . '</th><td><input type="tel" name="peracrm_whatsapp[business_phone_e164]" class="regular-text" value="' . esc_attr((string) ($settings['business_phone_e164'] ?? '')) . '" placeholder="+905xxxxxxxxx" /><p class="description">' . esc_html__('The business display number in E.164 format. Used to prevent the business itself becoming a CRM client.', 'peracrm') . '</p></td></tr>';
    echo '<tr><th scope="row">' . esc_html__('WABA ID', 'peracrm') . '</th><td><input type="text" name="peracrm_whatsapp[waba_id]" class="regular-text" value="' . esc_attr((string) $settings['waba_id']) . '" /><p class="description">' . esc_html__('Optional but recommended inbound account allow-list.', 'peracrm') . '</p></td></tr>';
    ...
    echo '<tr><th scope="row">' . esc_html__('Test mode', 'peracrm') . '</th><td><label><input type="checkbox" name="peracrm_whatsapp[test_mode]" value="1" ' . checked(!empty($settings['test_mode']), true, false) . ' /> ' . esc_html__('Label this configuration as test', 'peracrm') . '</label><p class="description">' . esc_html__('Informational only; enabling and complete credentials control production operation.', 'peracrm') . '</p></td></tr>';
    echo '</tbody></table>';
    submit_button(__('Save WhatsApp settings', 'peracrm'));
    echo '</form>';
    echo '</section>';

    echo '<section class="peracrm-whatsapp-admin__section peracrm-whatsapp-admin__section--enquiry-test">';
    echo '<h2>' . esc_html__('Enquiry notification test', 'peracrm') . '</h2>';
    echo '<form method="post" action="' . esc_url(admin_url('admin-post.php')) . '" class="peracrm-whatsapp-admin__form peracrm-whatsapp-admin__form--test">';
    ...
    echo '<section class="peracrm-whatsapp-admin__section peracrm-whatsapp-admin__section--messages">';
    ...
    echo '<table class="widefat striped peracrm-whatsapp-table peracrm-whatsapp-table--messages"><thead><tr>';
    ...
    if (empty($message_rows)) {
        echo '<tr><td colspan="6" data-label="' . esc_attr__('Status', 'peracrm') . '">' . esc_html__('No WhatsApp messages have been synced yet.', 'peracrm') . '</td></tr>';
    } else {
        foreach ($message_rows as $row) {
            ...
            echo '<tr>';
            echo '<td data-label="' . esc_attr__('Created', 'peracrm') . '">' . esc_html((string) ($row['created_at'] ?? '')) . '</td>';
            echo '<td data-label="' . esc_attr__('Direction', 'peracrm') . '">' . esc_html(ucfirst((string) ($row['direction'] ?? ''))) . '</td>';
            echo '<td data-label="' . esc_attr__('Client', 'peracrm') . '">' . $client_label . '</td>';
            echo '<td data-label="' . esc_attr__('Contact', 'peracrm') . '">' . (!empty($contact_parts) ? implode('<br />', $contact_parts) : '—') . '</td>';
            echo '<td data-label="' . esc_attr__('Phone', 'peracrm') . '"><code>' . esc_html((string) ($row['phone_e164'] ?? '')) . '</code></td>';
            echo '<td data-label="' . esc_attr__('Message', 'peracrm') . '">' . $message_preview . '</td>';
            echo '</tr>';
        }
    }
    ...
    echo '<section class="peracrm-whatsapp-admin__section peracrm-whatsapp-admin__section--notification-logs">';
    echo '<h2>' . esc_html__('Recent enquiry notification logs', 'peracrm') . '</h2>';
    echo '<table class="widefat striped peracrm-whatsapp-table peracrm-whatsapp-table--notifications"><thead><tr>';
    ...
    if (empty($notification_rows)) {
        echo '<tr><td colspan="8" data-label="' . esc_attr__('Status', 'peracrm') . '">' . esc_html__('No notification rows yet.', 'peracrm') . '</td></tr>';
    } else {
        foreach ($notification_rows as $row) {
            ...
            echo '<tr>';
            echo '<td data-label="' . esc_attr__('When', 'peracrm') . '">' . esc_html((string) ($row['created_at'] ?? '')) . '</td>';
            echo '<td data-label="' . esc_attr__('Event', 'peracrm') . '">' . esc_html((string) ($row['event_type'] ?? '')) . '</td>';
            echo '<td data-label="' . esc_attr__('Source', 'peracrm') . '">' . esc_html((string) ($row['source'] ?? '')) . '</td>';
            echo '<td data-label="' . esc_attr__('Client', 'peracrm') . '">' . $client_text . '</td>';
            echo '<td data-label="' . esc_attr__('Channel', 'peracrm') . '">' . esc_html((string) ($row['channel'] ?? '')) . '</td>';
            echo '<td data-label="' . esc_attr__('Recipient', 'peracrm') . '">' . esc_html((string) ($row['recipient'] ?? '')) . '</td>';
            echo '<td data-label="' . esc_attr__('Status', 'peracrm') . '">' . esc_html((string) ($row['status'] ?? '')) . '</td>';
            echo '<td data-label="' . esc_attr__('Provider', 'peracrm') . '">' . esc_html((string) ($row['provider_code'] ?? '')) . '</td>';
            echo '</tr>';
        }
    }
    echo '</tbody></table>';
