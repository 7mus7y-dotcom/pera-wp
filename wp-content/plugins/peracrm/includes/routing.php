...
if ( ! function_exists( 'peracrm_asset_ver' ) ) {
	function peracrm_asset_ver( string $rel_path ): string {
		$abs_path = PERACRM_PATH . '/' . ltrim( $rel_path, '/' );
		return file_exists( $abs_path ) ? (string) filemtime( $abs_path ) : ( defined( 'PERACRM_VERSION' ) ? (string) PERACRM_VERSION : '1.0.0' );
	}
}

if ( ! function_exists( 'peracrm_template_path' ) ) {
	function peracrm_template_path( string $template ): string {
		return PERACRM_PATH . '/templates/' . ltrim( $template, '/' );
	}
}

if ( ! function_exists( 'peracrm_locate_template' ) ) {
	function peracrm_locate_template( string $template ): string {
		$plugin_template = peracrm_template_path( $template );
		if ( file_exists( $plugin_template ) ) {
			return $plugin_template;
		}

		$allow_theme_fallback = (bool) apply_filters( 'peracrm_allow_theme_template_fallback', false, $template );
		if ( ! $allow_theme_fallback ) {
			return '';
		}

		$theme_template = locate_template( $template );
		return is_string( $theme_template ) ? $theme_template : '';
	}
}

if ( ! function_exists( 'peracrm_render_template_part' ) ) {
	function peracrm_render_template_part( string $template, array $args = array() ): void {
		$template_path = peracrm_locate_template( $template );
		if ( '' === $template_path ) {
			return;
		}
...
if ( ! function_exists( 'pera_crm_handle_new_lead' ) ) {
	...
	function pera_crm_handle_new_lead(): void {
		...
		if ( ! pera_crm_user_can_access() || ! $mu_can_access || ! current_user_can( 'edit_crm_clients' ) ) {
			wp_die( esc_html__( 'You are not allowed to create CRM leads.', 'peracrm' ), 'Forbidden', array( 'response' => 403 ) );
		}
		...
		if ( $existing_client_id > 0 ) {
			pera_crm_set_flash_message( __( 'A lead with this email already exists.', 'peracrm' ), 'warning' );
...
if ( ! function_exists( 'pera_crm_gate_or_redirect' ) ) {
	...
	function pera_crm_gate_or_redirect(): void {
		...
		if ( ! pera_crm_user_can_access() ) {
			...
			wp_die( esc_html__( 'You are not allowed to access this page.', 'peracrm' ), 'Forbidden', array( 'response' => 403 ) );
		}
	}
}
...
if ( ! function_exists( 'pera_crm_maybe_load_template' ) ) {
	...
	function pera_crm_maybe_load_template( string $template ): string {
		...
		if ( 'new' === $action ) {
			$new_template = peracrm_locate_template( 'page-crm-new.php' );
		...
		if ( 'client' === $view ) {
			$client_template = peracrm_locate_template( 'page-crm-client.php' );
		...
		if ( 'pipeline' === $view ) {
			$pipeline_template = peracrm_locate_template( 'page-crm-pipeline.php' );
		...
		$crm_template = peracrm_locate_template( 'page-crm.php' );
...
if ( ! function_exists( 'pera_crm_enqueue_assets' ) ) {
	...
	function pera_crm_enqueue_assets(): void {
		...
		$crm_css_deps = array();
		if ( wp_style_is( 'pera-main-css', 'registered' ) ) {
			$crm_css_deps[] = 'pera-main-css';
		}
		if ( wp_style_is( 'pera-slider-css', 'registered' ) ) {
			$crm_css_deps[] = 'pera-slider-css';
		}

		wp_enqueue_style(
			'pera-crm-css',
			PERACRM_URL . '/assets/css/crm.css',
			$crm_css_deps,
			peracrm_asset_ver( '/assets/css/crm.css' )
		);

		wp_enqueue_script(
			'pera-crm-js',
			PERACRM_URL . '/assets/js/crm.js',
			array(),
			peracrm_asset_ver( '/assets/js/crm.js' ),
			true
		);
...
if ( ! function_exists( 'pera_crm_rewrite_notice' ) ) {
	...
	function pera_crm_rewrite_notice(): void {
		...
		echo esc_html__( 'CRM routes changed? Save Permalinks once (or run wp rewrite flush --hard) to refresh /crm/* rewrites. This is not auto-flushed per request.', 'peracrm' );
		echo '</p></div>';
	}
}
