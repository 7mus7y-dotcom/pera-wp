 ...
 *
 * Run with: wp eval-file wp-content/plugins/pera-multilingual/tools/register-theme-ui-strings.php
 * Dry run:  wp eval-file wp-content/plugins/pera-multilingual/tools/register-theme-ui-strings.php -- --dry-run
 */
...
final class Pera_ML_Theme_UI_Discovery {
	...
	public function __construct( Pera_ML_UI_Registry $registry ) {
	...
	public function run( array $directories, $dry_run = false ) {
		...
		foreach ( $this->php_files( $directories ) as $file ) {
			...
			foreach ( $parsed['registrations'] as $registration ) {
				list( $source, $key ) = $registration;
				$stats['discovered']++;
				$identity = Pera_ML_UI::identity( $source, $key );
				$existing = $this->registry->find( $identity );
				if ( ! $existing ) {
					$stats['newly_registered']++;
				} elseif ( ! isset( $existing['source_hash'] ) || ! hash_equals( (string) $existing['source_hash'], hash( 'sha256', $source ) ) ) {
		...
		foreach ( $directories as $directory ) {
			...
			$iterator = new RecursiveIteratorIterator(
				new RecursiveCallbackFilterIterator(
					...
					function ( $item ) { return ! $item->isDir() || '_archive' !== $item->getFilename(); }
				)
			);
	...
	/** @return array{registrations:array<int,array{string,string}>,dynamic_skipped:int} */
	private function parse( $php ) {
		$tokens = token_get_all( (string) $php );
		$registrations = array();
		$skipped = 0;
		for ( $i = 0, $count = count( $tokens ); $i < $count; $i++ ) {
			...
			$open = $this->next_significant_index( $tokens, $i + 1 );
			if ( null === $open || '(' !== $tokens[ $open ] ) continue;

			$first = $this->next_significant_index( $tokens, $open + 1 );
			$comma = null === $first ? null : $this->next_significant_index( $tokens, $first + 1 );
			$second = null === $comma ? null : $this->next_significant_index( $tokens, $comma + 1 );
			$after = null === $second ? null : $this->next_significant_index( $tokens, $second + 1 );
			$valid_shape = null !== $first && null !== $comma && ',' === $tokens[ $comma ] && null !== $second && null !== $after && in_array( $tokens[ $after ], array( ')', ',' ), true );
			if ( ! $valid_shape || ! $this->is_literal( $tokens[ $first ] ) || ! $this->is_literal( $tokens[ $second ] ) ) {
				$skipped++;
				continue;
			}
			$source = $this->literal_value( $tokens[ $first ][1] );
			$key = $this->literal_value( $tokens[ $second ][1] );
			if ( 0 !== strpos( $key, 'theme.' ) ) continue;
			$registrations[] = array( $source, $key );
		}
	...
	private function literal_value( $literal ) {
		...
		return "'" === $quote ? str_replace( array( "\\\\", "\\'" ), array( "\\", "'" ), $body ) : stripcslashes( $body );
	}

	private function next_significant_index( array $tokens, $index ) {
		for ( $count = count( $tokens ); $index < $count; $index++ ) {
			if ( ! is_array( $tokens[ $index ] ) || ! in_array( $tokens[ $index ][0], array( T_WHITESPACE, T_COMMENT, T_DOC_COMMENT ), true ) ) return $index;
		}
		return null;
	}

	private function previous_significant( array $tokens, $index ) {
		for ( ; $index >= 0; $index-- ) {
			if ( ! is_array( $tokens[ $index ] ) || ! in_array( $tokens[ $index ][0], array( T_WHITESPACE, T_COMMENT, T_DOC_COMMENT ), true ) ) return $tokens[ $index ];
		}
		return null;
	}
}
...
if ( ! defined( 'PERA_ML_THEME_UI_DISCOVERY_NO_RUN' ) ) {
	...
	$stats = ( new Pera_ML_Theme_UI_Discovery( new Pera_ML_UI_Registry() ) )->run(
		array( $theme . '/inc', $theme . '/partials', $theme . '/parts' ),
		$dry_run
	);
	$labels = array(
