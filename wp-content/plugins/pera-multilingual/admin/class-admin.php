...
final class Pera_ML_Admin {
	...
	public function enqueue_translation_queue( $hook ) { if ( ! in_array( $hook, array( 'post.php', 'post-new.php', 'term.php' ), true ) ) return; $path = PERA_ML_DIR . 'admin/translation-queue.js'; $version = file_exists( $path ) ? (string) filemtime( $path ) : PERA_ML_VERSION; wp_enqueue_script( 'pera-ml-admin-queue', PERA_ML_URL . 'admin/translation-queue.js', array(), $version, true ); wp_localize_script( 'pera-ml-admin-queue', 'peraMLQueue', array( 'ajaxUrl' => admin_url( 'admin-ajax.php' ) ) ); }
	...
	private function term_health() { $plugin = Pera_ML_Plugin::instance(); return new Pera_ML_Translation_Health( $plugin->status(), $plugin->storage(), $plugin->ui(), $this->registry ); }
	...
	public function term_ajax_request() {
		...
		if ( ! $term_id ) return new WP_Error( 'invalid_term' );
		$term = get_term( $term_id, $taxonomy );
		if ( ! $term instanceof WP_Term || $taxonomy !== $term->taxonomy ) return new WP_Error( 'invalid_term' );
		if ( ! current_user_can( 'edit_term', $term_id ) ) return new WP_Error( 'insufficient_capability' );
	...
	public function translate_term_field( $term, $taxonomy, $language, $field, $mode = 'complete', $health = null, $orchestrator = null ) {
		...
		if ( ! $orchestrator ) { $plugin = Pera_ML_Plugin::instance(); $orchestrator = new Pera_ML_Translation_Health_Orchestrator( $plugin->status(), $plugin->storage(), $plugin->translator(), $plugin->ui(), $plugin->ui_registry(), $this->registry ); }
	...
	public function translation_health_page() {
		if ( ! current_user_can( 'manage_options' ) ) return;
		$health = new Pera_ML_Translation_Health( Pera_ML_Plugin::instance()->status(), Pera_ML_Plugin::instance()->storage(), Pera_ML_Plugin::instance()->ui(), $this->registry );
	...
	public function ajax_health_translate() {
		...
		$orchestrator = new Pera_ML_Translation_Health_Orchestrator( $plugin->status(), $plugin->storage(), $plugin->translator(), $plugin->ui(), $plugin->ui_registry(), $this->registry );
