...
final class Pera_ML_Translation_Health_Orchestrator {
	...
	public function translate( array $row ) {
		if ( ! isset( $row['object_type'], $row['object_id'], $row['field'], $row['language'], $row['status'] ) || ! in_array( $row['language'], Pera_ML_Translation_Health::LANGUAGES, true ) || ! in_array( $row['status'], array( 'missing', 'stale', 'regenerate' ), true ) ) return new WP_Error( 'invalid_row' );
		...
		if ( 0 === strpos( $type, 'taxonomy:' ) ) return $this->translate_term( substr( $type, 9 ), $id, $field, $language, 'regenerate' === $row['status'] );
