...
function peracrm_ingest_elementor_enquiry($record, $ajax_handler)
{
    ...
    foreach ($raw_fields as $field_id => $field_data) {
        if (is_array($field_data) && isset($field_data['value'])) {
            $flat[(string) $field_id] = is_array($field_data['value'])
                ? implode(', ', array_map('sanitize_text_field', $field_data['value']))
                : sanitize_text_field((string) $field_data['value']);
        }
    }

    $name = $flat['name'] ?? ($flat['full_name'] ?? '');
    [$first_name, $last_name] = peracrm_ingest_split_name($name);

    $payload = [
        'form_context' => 'elementor',
        'name' => $name,
        'first_name' => $first_name,
        'last_name' => $last_name,
        'email' => $flat['email'] ?? '',
        'phone' => $flat['phone'] ?? ($flat['telephone'] ?? ''),
        'message' => $flat['message'] ?? '',
        'preferred_contact' => $flat['preferred_contact'] ?? '',
        'property_id' => isset($flat['property_id']) ? absint($flat['property_id']) : 0,
        'page_url' => $flat['page_url'] ?? ($flat['referrer'] ?? ''),
        'raw_fields' => $flat,
    ];

    $form_settings = method_exists($record, 'get_form_settings') ? (array) $record->get_form_settings() : [];
    ...
    peracrm_ingest_enquiry($payload, peracrm_ingest_request_context('elementor_form', $form_id, $form_name));
}

function peracrm_ingest_cf7_enquiry($contact_form)
{
    ...
    $name = isset($posted['your-name']) ? sanitize_text_field((string) $posted['your-name']) : '';
    [$first_name, $last_name] = peracrm_ingest_split_name($name);

    $payload = [
        'form_context' => 'contact_form_7',
        'name' => $name,
        'first_name' => $first_name,
        'last_name' => $last_name,
        'email' => isset($posted['your-email']) ? sanitize_email((string) $posted['your-email']) : '',
        'phone' => isset($posted['your-phone']) ? sanitize_text_field((string) $posted['your-phone']) : '',
        'message' => isset($posted['your-message']) ? sanitize_textarea_field((string) $posted['your-message']) : '',
        'property_id' => isset($posted['property_id']) ? absint($posted['property_id']) : 0,
        'page_url' => isset($posted['_wpcf7_container_post']) ? get_permalink((int) $posted['_wpcf7_container_post']) : '',
        'raw_fields' => $posted,
    ];
    ...
    peracrm_ingest_enquiry($payload, peracrm_ingest_request_context('contact_form_7', $form_id, $form_name));
}

function peracrm_register_optional_form_integrations()
{
...
add_action('plugins_loaded', 'peracrm_register_optional_form_integrations', 20);
