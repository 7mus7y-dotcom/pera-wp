...
if ( ! function_exists( 'pera_get_district_page_faqs' ) ) {
  /**
   * Return validated district FAQ rows from ACF term field.
   *
   * @param WP_Term|null $term Optional district term. Defaults to queried term.
   * @return array<int,array{question:string,answer:string,answer_plain:string}>
   */
  function pera_get_district_page_faqs( ?WP_Term $term = null ): array {
    ...
    if ( ! ( $term instanceof WP_Term ) || $term->taxonomy !== 'district' || (int) $term->term_id <= 0 ) {
      return array();
    }

    $rows = get_field( 'district_page_faqs', 'district_' . (int) $term->term_id );
    ...
    foreach ( $rows as $row ) {
      ...
      $answer_plain = trim( preg_replace( '/\s+/', ' ', wp_strip_all_tags( $answer ) ) );
...
if ( ! function_exists( 'pera_render_district_page_faqs' ) ) {
  /**
   * Render district FAQ accordion on district archive pages.
   *
   * @param WP_Term|null $term Optional district term. Defaults to queried term.
   */
  function pera_render_district_page_faqs( ?WP_Term $term = null ): void {
    if ( ! ( $term instanceof WP_Term ) ) {
      $queried = get_queried_object();
      $term    = ( $queried instanceof WP_Term ) ? $queried : null;
    }

    if ( ! ( $term instanceof WP_Term ) || $term->taxonomy !== 'district' ) {
      return;
    }

    $faq_items = pera_get_district_page_faqs( $term );
    if ( empty( $faq_items ) ) {
      return;
    }
    ?>
    <section class="section">
      <div class="container">
        <div class="article-body">
          <h2><?php echo esc_html( sprintf( 'Frequently Asked Questions About %s', $term->name ) ); ?></h2>
          <?php foreach ( $faq_items as $faq_item ) : ?>
            <details class="card-shell mb-sm">
              <summary><strong><?php echo esc_html( $faq_item['question'] ); ?></strong></summary>
...
if ( ! function_exists( 'pera_render_district_page_faq_schema' ) ) {
  /**
   * Output FAQPage JSON-LD for district archive FAQs.
   *
   * @param WP_Term|null $term Optional district term. Defaults to queried term.
   */
  function pera_render_district_page_faq_schema( ?WP_Term $term = null ): void {
    if ( ! ( $term instanceof WP_Term ) ) {
      $queried = get_queried_object();
      $term    = ( $queried instanceof WP_Term ) ? $queried : null;
    }
