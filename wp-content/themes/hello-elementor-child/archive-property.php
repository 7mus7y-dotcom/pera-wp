...
// Do not use get_permalink() on archive templates: it can resolve to the loop's global $post URL.
$archive_base_url = trim( $archive_base_candidate ) !== ''
...
$ctx = array(
  ...
  'has_price_qs'               => $has_price_qs,
  'qs_min'                     => $qs_min,
  'qs_max'                     => $qs_max,
  'sort'                       => $sort,
);

$args = pera_property_archive_build_args_from_context( $ctx );
...
if ( $qo instanceof WP_Term ) {
  ...
  } elseif ( trim( wp_strip_all_tags( $generic_archive_body ) ) !== '' ) {
...
if ( ! $is_filtered_search && ( $qo instanceof WP_Term ) && ! is_wp_error( $qo ) ) {
  ...
  } elseif ( trim( wp_strip_all_tags( $generic_archive_subtitle ) ) !== '' ) {
    $hero_desc_html = '<p class="text-light">' . esc_html( trim( $generic_archive_subtitle ) ) . '</p>';
...
<?php if ( function_exists( 'pera_should_render_property_taxonomy_archive_cta' ) && pera_should_render_property_taxonomy_archive_cta( $qo ) ) : ?>
  <?php
    ...
    $taxonomy_archive_cta_contact_url      = function_exists( 'pera_ml_url' ) ? pera_ml_url( home_url( '/contact-us/' ) ) : home_url( '/contact-us/' );
    $taxonomy_archive_cta_all_property_url = get_post_type_archive_link( 'property' );

    if ( ! $taxonomy_archive_cta_all_property_url ) {
      $taxonomy_archive_cta_all_property_url = function_exists( 'pera_ml_url' ) ? pera_ml_url( home_url( '/property/' ) ) : home_url( '/property/' );
    }
  ?>
    ...
    <section class="archive-cta section section-soft">
      <div class="container">
        ...
        <div class="article-cta-actions">
          ...
          <a class="btn btn--solid btn--blue" href="<?php echo esc_url( function_exists( 'pera_ml_url' ) ? pera_ml_url( home_url( '/book-a-consultancy/' ) ) : home_url( '/book-a-consultancy/' ) ); ?>">
