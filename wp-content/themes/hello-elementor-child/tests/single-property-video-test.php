...
expect_property_video( false !== strpos( $template, '<div class="property-hero__video-action property-hero__cta">' ), 'video action reuses the existing hero CTA layout' );
expect_property_video( false !== strpos( $template, 'class="btn btn--ghost btn--white"' ), 'hero video button uses exactly the established visual classes' );
expect_property_video( false === strpos( $template, 'property-hero__video-button' ), 'bespoke video button class is absent' );
expect_property_video( false === strpos( $template, 'property-hero__video-icon' ), 'bespoke video icon class is absent' );
expect_property_video( false !== strpos( $template, 'class="icon" aria-hidden="true" focusable="false"' ), 'play icon uses the generic icon class and is decorative' );
...
$video_action_position = strpos( $template, '<div class="property-hero__video-action property-hero__cta">' );
$cta_position          = strpos( $template, '<div class="property-hero__cta">', $video_action_position + 1 );
...
expect_property_video( false === strpos( $styles, '.property-hero__video-action' ), 'video action has no dedicated CSS declaration' );
expect_property_video( false === strpos( $styles, 'property-hero__video-button' ), 'video button has no padding, margin, height, colour, hover, or focus overrides' );
expect_property_video( false === strpos( $styles, 'property-hero__video-icon' ), 'video icon has no dedicated CSS declaration' );
